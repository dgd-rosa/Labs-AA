...
import warnings
from sklearn.metrics import confusion_matrix, accuracy_score, balanced_accuracy_score, f1_score, plot_roc_curve
#Model selection
...
def naive_bayes(x1_train, y1_train, x1_test, y1_test, count):
    ...
    print("Accuracy Score in the training set [Over/Underfitting]: {0}".format(nb_train_acc_score))
    metrics(y1_test, y_pred_nb, y_pred_prob_nb)
    #plotting roc curve
    plot_roc_curve(nb_model,x1_test,y1_test)
    plt.plot([0,1],[0,1],'k--')
    plt.title('ROC Curve and AUC for NAIVE')
    plt.legend()
...
def svm_poly(x1_train, y1_train, x1_test, y1_test):   
    ...
    poly_svm = SVC(kernel='poly', degree=clf.best_params_['degree'], 
                   class_weight=clf.best_params_['class_weight'], max_iter=100000, probability=True)
    ...
    print("Accuracy Score in the training set [Over/Underfitting]: ", poly_train_acc)
    ...
    plt.plot([0,1],[0,1],'k--')
    plt.title('ROC Curve and AUC for Polynomial')
    plt.legend()
...
def svm_linear(x1_train, y1_train, x1_test, y1_test):    
    ...
    print("Accuracy Score in the training set [Over/Underfitting]: ", lin_train_acc)
    ...
    plt.plot([0,1],[0,1],'k--')
    plt.title('ROC Curve and AUC for Linear')
    plt.legend()
...
def svm_rbf(x1_train, y1_train, x1_test, y1_test):   
    ...
    plt.plot([0,1],[0,1],'k--')
    plt.title('ROC Curve and AUC for Gaussian RBF')
    plt.legend()
